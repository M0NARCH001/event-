export interface EventFormData {
  ...
  entrySide: string; // Renamed from countryside
  ticketType: string;
  audienceCategory: { category: string; price: string; description: string }[];
  refundPolicy: string;
  ticketName: string;
  ticketQuantity: string; // or number (stored as string in input)
  ...
  sponsors: { // Changed to object based on usage in EventPage/SponsorshipForm
  ...
  } | { name: string; website: string; type: string }[]; // Handle potential array legacy structure if needed, but code prefers object
  requirements: {
    artists: string;
  ...
  // Add other fields as necessary from initial state
  transportOptions: {
  ...
  media: string;
  idleness: string;
  titleSponsors?: any; // legacy
  coPartners?: any; // legacy
  mediaPartners?: any; // legacy
  type?: string;
  ticketPrice?: string;
