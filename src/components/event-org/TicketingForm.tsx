                                    ...
                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', width: '100%' }}>
                                        {addOnOptions.map((option) => (
                                            <div
                                                ...
                                                onKeyDown={(e) => e.key === 'Enter' && handleAddOnChange({ target: { name: `addOns.${option.id}`, checked: !formData.addOns[option.id] } } as any)}
                                                role="button"
                                                tabIndex={0}
                                                aria-checked={!!formData.addOns[option.id]}
                                            >
                                                <input
                                                    type="checkbox"
                                                    id={`addOn-${option.id}`}
                                                    name={`addOns.${option.id}`}
                                                    checked={!!formData.addOns[option.id]}
                                                    onChange={handleAddOnChange}
                                                    style={{ width: '16px', height: '16px', cursor: 'pointer', margin: 0, padding: 0 }}
                                                />
                                                <label
                                                    htmlFor={`addOn-${option.id}`}
                                                    style={{ fontSize: '12px', fontWeight: 500, color: 'black', cursor: 'pointer', lineHeight: '16px' }}
                                                >
                                                    {option.label}
                                                </label>
                                            </div>
